        ...
        try {
            ...
            if(rowCountItems == 1)
            {
                ...
                if(applyJoiningCredit)
                {
                    ...
                    if(user.getRole()== Constants.ROLE_SHOP_ADMIN_CODE)
                    {
                        statement.setObject(++i, marketSettingsDAO.getSettingsInstance().getJoiningCreditForShopOwner());
                    }
                    else if(user.getRole()== Constants.ROLE_END_USER_CODE)
                    {
                        statement.setObject(++i, marketSettingsDAO.getSettingsInstance().getJoiningCreditForEndUser());
                    }
                    else
                    {
                ...
                if(applyReferralCredit)
                {
                    ...
                    if(user.getRole()== Constants.ROLE_END_USER_CODE)
                    {
                        statement.setObject(++i, marketSettingsDAO.getSettingsInstance().getReferralCreditForEndUserRegistration());
                    }
                    else
                    {
                        statement.setObject(++i,0);
