...
//				+ Shop.TIMESTAMP_CREATED + ","
		...
		try {
			...
			statement.setObject(++i, Constants.ROLE_SHOP_ADMIN_CODE);
	...
	public int deleteShop(int shopID)
	{
		...
		int shopAdminID = daoUserUtility.getUserIDforShopAdmin(shopID);
...
//			System.out.println("Total rows updated: " + updatedRows);
	...
	public int updateShopByAdmin(Shop shop)
	{
		...
		try {
			...
			statement.setObject(++i,shop.isItemUpdatePermitted());
			statement.setObject(++i,shop.isOpen());
...
//			System.out.println("Total rows updated: " + updatedRows);
	...
	public Shop getShopPhoto(int ShopID)
	{
...
//		}
	...
	public Shop getShopDetailsForOrder(int ShopID,
							   Double latCenter, Double lonCenter)
	{
...
//
//
//	public boolean isItemUpdatePermitted(int ShopID)
//	{
//
//		String query = " ";
//
//
//		query = "SELECT " + Shop.TABLE_NAME + "." + Shop.ITEM_UPDATE_PERMITTED + ""
//				+ " FROM " + Shop.TABLE_NAME
//				+ " WHERE "	+ Shop.TABLE_NAME + "." + Shop.SHOP_ID + "= " + ShopID;
//
//
...
//	}
	...
	public Shop getShopDetailsForPlaceOrderPage(int ShopID,
												Double latCenter, Double lonCenter)
	{
		...
		try {
			...
			while(rs.next())
			{
				...
				shop.setShopID(rs.getInt(Shop.SHOP_ID));
				shop.setBillAmountForFreeDelivery(rs.getInt(Shop.BILL_AMOUNT_FOR_FREE_DELIVERY));
				shop.setDeliveryCharges(rs.getInt(Shop.DELIVERY_CHARGES));
				shop.setPickFromShopAvailable(rs.getBoolean(Shop.PICK_FROM_SHOP_AVAILABLE));
				shop.setHomeDeliveryAvailable(rs.getBoolean(Shop.HOME_DELIVERY_AVAILABLE));
			}
	...
	public Shop getShopDetailsForCartsList(int ShopID,
									   Double latCenter, Double lonCenter)
	{
...
//			System.out.println("Total Shops queried " + shopList.size());
	...
	)
	{
		...
		queryJoin = "SELECT "
				...
				+ " AND " + Shop.TABLE_NAME + "." + Shop.ACCOUNT_BALANCE + ">=" + marketSettingsDAO.getSettingsInstance().getJoiningCreditForShopOwner();
...
//					shop.setShortDescription(rs.getString(Shop.SHORT_DESCRIPTION));
					...
					if(marketSettingsDAO.getSettingsInstance().isBootstrapModeEnabled())
					{
						shop.setRt_rating_avg(UtilityMethods.getRandomIntegerBetweenRange(3,5));
						shop.setRt_rating_count(UtilityMethods.random.nextInt(500));
					}
